...
description: "Use when the user explicitly asks for Tarot, fate, or card-based tie-breaking, or when an existing brainstorming session has already handed off to this skill with a transfer prompt."
allowed-tools:
  - Bash
  - Read
  - Grep
...
`let-fate-decide` is not the default front door for ambiguous creative work. `brainstorming` owns that flow. Use this skill only when the user explicitly wants Tarot/fate-based guidance, or when `brainstorming` has already offered that detour and sent the user here with a transfer prompt.
...
- **Explicit invocations**: "I'm feeling lucky", "let fate decide", "dealer's choice", "surprise me", "YOLO", "heart of the cards"
- **Brainstorming handoff**: The user arrives with a transfer prompt from an earlier brainstorming conversation asking for a reading to break a tie
...
- **Tie-breaking after consent**: The user has made clear that they want fate/Tarot to break the tie between 2+ valid approaches

## When NOT to Use

- The user is merely vague, casual, or under-specified but has not asked for Tarot/fate
- The work is still in early ideation and should be handled by `brainstorming`
...
- You are about to silently replace clarifying questions or normal design work with a card draw
...
```
User: "Let fate decide which direction we should take here"
...
|----------------|-----------|
| "The user was vague, so I can skip brainstorming" | Vagueness alone is not permission; brainstorming owns ambiguous creative work |
